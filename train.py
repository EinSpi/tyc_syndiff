...
def train_log(iteration,global_step,epoch,
            errG,errG_L1,errG_cycle,errG_adv,errG_cycle_adv,
            errD,errD_fake,errD_real,errD_cycle,errD_cycle_fake,errD_cycle_real):
    # Where the magic happens
    wandb.log({"iteration": iteration,"epoch": epoch,
                "errG": errG,"errG_L1":errG_L1,"errG_cycle":errG_cycle,"errG_adv":errG_adv,"errG_cycle_adv":errG_cycle_adv,
                "errD": errD,"errD_fake":errD_fake,"errD_real":errD_real,"errD_cycle":errD_cycle,"errD_cycle_fake":errD_cycle_fake,"errD_cycle_real":errD_cycle_real}, step=global_step)
    print(f"Loss after {str(global_step).zfill(5)} examples:")
...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            #creating experiment path
            #exp_path = os.path.join(opt.output_path,opt.exp)
            #torchvision.utils.save_image(x_0[:,[0],:], os.path.join(exp_path, 'x_0_of_timestep{}.png'.format(i)), normalize=True)
...
def train_syndiff(rank, gpu, args):
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(dataset,
                                                                    num_replicas=args.num_proc_node * args.num_process_per_node,
                                                                    rank=rank)
    ...
    print("reached checkpoint4")
    exp = args.exp
    #yichen dynamically modify the output path according to hyperparameters
    output_path = os.path.join(args.output_path,
                               'num_channels_dae='+str(args.num_channels_dae)+'num_timesteps='+str(args.num_timesteps))
    #creating experiment path
    ...
    print("reached checkpoint5")
    coeff = Diffusion_Coefficients(args, device)
    pos_coeff = Posterior_Coefficients(args, device)
    T = get_time_schedule(args, device)
    print("reached checkpoint6")
    if args.resume:
    ...
    print("reached checkpoint7")
    #yichen 's code for watching gradient for 8 blocks
    wandb.watch(models=(gen_diffusive_1,gen_diffusive_2,gen_non_diffusive_1to2,gen_non_diffusive_2to1), log="all", log_freq=20)
    
    #modified by yichen, originally for epoch in range(init_epoch, args.num_epoch+1):
    for epoch in range(init_epoch, args.num_epoch+init_epoch):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            ...
            if global_step%20==0:
                train_log(iteration,global_step,epoch,
                          errG.item(),errG_L1.item(),errG_cycle.item(),errG_adv.item(),errG_cycle_adv.item(),
                          errD.item(),errD_fake.item(),errD_real.item(),errD_cycle.item(),errD_cycle_fake.item(),errD_cycle_real.item()
                          )
            global_step += 1

        print("reached checkpoint8")
        ...
        print("reached checkpoint9")
 
        if rank == 0:
            if epoch % args.save_content_every == 0:
                #single step denoising all lead by real_data2
                torchvision.utils.save_image(x2_tp1,            os.path.join(exp_path,'x2_tp1_based_on_real_data2_epoch_{}.png').format(epoch),                 normalize=True)
                torchvision.utils.save_image(x2_t,              os.path.join(exp_path,'x2_t_based_on_real_data2_epoch_{}.png').format(epoch),                   normalize=True)
                torchvision.utils.save_image(x2_pos_sample,     os.path.join(exp_path,'x2_t_predicted_by_model_from_x2_tp1_epoch_{}.png'.format(epoch)),    normalize=True)

                #multiple step denoising(sampling). use real_data2 as guidance directly sample from model, #all lead by real_data2
                real_data2_as_guidance = torch.cat((torch.randn_like(real_data1),real_data2),axis=1)
                sample2to1,x0_1_tilda = sample_from_model(pos_coeff, gen_diffusive_1, args.num_timesteps, real_data2_as_guidance, T, args)
                torchvision.utils.save_image(real_data2,        os.path.join(exp_path,'real_data2_epoch_{}.png'.format(epoch)),                             normalize=True)
                torchvision.utils.save_image(sample2to1,        os.path.join(exp_path,'sample2to1_epoch_{}.png'.format(epoch)),                             normalize=True)
                torchvision.utils.save_image(x0_1_tilda,        os.path.join(exp_path,'x0_1_tilda_epoch_{}.png'.format(epoch)),                             normalize=True)
                model_results_from_real_data2 = torch.cat((real_data2, sample2to1,x0_1_tilda),axis=-1)
                torchvision.utils.save_image(model_results_from_real_data2, os.path.join(exp_path, 'model_results_from_real_data2_epoch_{}.png'.format(epoch)), normalize=False)
                #following includes no sampling, just quick modality switches using generators
                #first 3 are cycleing transform using non diffusive generators,the fourth is output of diffusive generator, all lead or derived by real data2
                cycle2to1 = gen_non_diffusive_2to1(real_data2)
                reverse_cycle1to2=gen_non_diffusive_1to2(cycle2to1)
                cycle2to1_as_guidance = torch.cat((torch.randn_like(real_data2), cycle2to1),axis=1)
                x0_2_tilda_fake = gen_diffusive_2(cycle2to1_as_guidance , t2, latent_z2)[:,[0],:]
                torchvision.utils.save_image(cycle2to1,         os.path.join(exp_path, 'cycle2to1_epoch_{}.png'.format(epoch)),                             normalize=True)
                torchvision.utils.save_image(reverse_cycle1to2, os.path.join(exp_path, 'reverse_cycle1to2_epoch_{}.png'.format(epoch)),                     normalize=True)
                torchvision.utils.save_image(x0_2_tilda_fake,   os.path.join(exp_path, 'x0_2_tilda_fake_epoch_{}.png'.format(epoch)),                       normalize=True)
                cycle_results_from_real_data2 = torch.cat((real_data2, cycle2to1, reverse_cycle1to2, x0_2_tilda_fake),axis=-1)
                torchvision.utils.save_image(cycle_results_from_real_data2, os.path.join(exp_path, 'cycle_results_from_real_data2_epoch_{}.png'.format(epoch)), normalize=True)
            
                #mirror codes------------------------------------------

                #single step denoising all lead by real_data1
                torchvision.utils.save_image(x1_tp1,            os.path.join(exp_path,'x1_tp1_based_on_real_data1_epoch_{}.png').format(epoch),                 normalize=True)
                torchvision.utils.save_image(x1_t,              os.path.join(exp_path,'x1_t_based_on_real_data1_epoch_{}.png').format(epoch),                   normalize=True)
                torchvision.utils.save_image(x1_pos_sample,     os.path.join(exp_path,'x1_t_predicted_by_model_from_x1_tp1_epoch_{}.png'.format(epoch)),    normalize=True)


                #multiple step denoising(sampling). use real_data2 as guidance directly sample from model, #all lead by real_data2
                real_data1_as_guidance = torch.cat((torch.randn_like(real_data2),real_data1),axis=1)
                sample1to2,x0_2_tilda = sample_from_model(pos_coeff, gen_diffusive_2, args.num_timesteps, real_data1_as_guidance, T, args)
                torchvision.utils.save_image(real_data1,        os.path.join(exp_path, 'real_data1_epoch_{}.png'.format(epoch)),                            normalize=True)
                torchvision.utils.save_image(sample1to2,        os.path.join(exp_path, 'sample1to2_epoch_{}.png'.format(epoch)),                            normalize=True)
                torchvision.utils.save_image(x0_2_tilda,        os.path.join(exp_path, 'x0_2_tilda_epoch_{}.png'.format(epoch)),                            normalize=True)
                model_results_from_real_data1 = torch.cat((real_data1, sample1to2,x0_2_tilda),axis=-1)
                torchvision.utils.save_image(model_results_from_real_data1, os.path.join(exp_path, 'model_results_from_real_data1_epoch_{}.png'.format(epoch)), normalize=False)
                #following includes no sampling, just quick modality switches using generators
                #first 3 are cycleing transform using non diffusive generators,the fourth is output of diffusive generator, all lead or derived by real data1
                cycle1to2 = gen_non_diffusive_1to2(real_data1)
                reverse_cycle2to1=gen_non_diffusive_2to1(cycle1to2)
                cycle1to2_as_guidance = torch.cat((torch.randn_like(real_data1), cycle1to2),axis=1)
                x0_1_tilda_fake = gen_diffusive_1(cycle1to2_as_guidance , t1, latent_z1)[:,[0],:]
                torchvision.utils.save_image(cycle1to2,         os.path.join(exp_path, 'cycle1to2_epoch_{}.png'.format(epoch)),                             normalize=True)
                torchvision.utils.save_image(reverse_cycle2to1, os.path.join(exp_path, 'reverse_cycle2to1_epoch_{}.png'.format(epoch)),                     normalize=True)
                torchvision.utils.save_image(x0_1_tilda_fake,   os.path.join(exp_path, 'x0_1_tilda_fake_epoch_{}.png'.format(epoch)),                       normalize=True)
                cycle_results_from_real_data1 = torch.cat((real_data1, cycle1to2, reverse_cycle2to1, x0_1_tilda_fake),axis=-1)
                torchvision.utils.save_image(cycle_results_from_real_data1, os.path.join(exp_path, 'cycle_results_from_real_data1_epoch_{}.png'.format(epoch)), normalize=True)
                if args.save_content:
                    print('Saving content.')
                    content = {'epoch': epoch + 1, 'global_step': global_step, #'args': args, modified yichen because wandb.config instance not serializable to be saved on disk
        ...
        for iteration, (x_val , y_val) in enumerate(data_loader_val): 
            ...
            #diffusion steps
            fake_sample1,_ = sample_from_model(pos_coeff, gen_diffusive_1, args.num_timesteps, x1_t, T, args)            
            ...
            val_l1_loss[0,epoch-init_epoch,iteration]=abs(fake_sample1 -real_data).mean()#yichen modified
            
            val_psnr_values[0,epoch-init_epoch, iteration] = psnr(real_data,fake_sample1, data_range=real_data.max())

        print("reached checkpoint11")
        ...
        for iteration, (y_val , x_val) in enumerate(data_loader_val): 
            ...
            fake_sample1=fake_sample1.cpu().numpy()
            real_data=real_data.cpu().numpy()
            val_l1_loss[1,epoch-init_epoch,iteration]=abs(fake_sample1 -real_data).mean()
            
            val_psnr_values[1,epoch-init_epoch, iteration] = psnr(real_data,fake_sample1, data_range=real_data.max())

        print("reached checkpoint12")

        print(np.nanmean(val_psnr_values[0,epoch-init_epoch,:]))
        print(np.nanmean(val_psnr_values[1,epoch-init_epoch,:]))
        np.save('{}/val_l1_loss.npy'.format(exp_path), val_l1_loss)
        np.save('{}/val_psnr_values.npy'.format(exp_path), val_psnr_values)

        print("reached checkpoint13")



def init_processes(l_rank, rank, size, fn, args):
    """ Initialize the distributed environment. """
    os.environ['MASTER_ADDR'] = os.getenv('SLURM_NODELIST').split()[0]#yichen
    os.environ['MASTER_PORT'] = '12355'#yichen
    torch.cuda.set_device(l_rank)
    gpu = l_rank
    print("before calling init_process_group")
    #modified here from nccl to gloo
    #modified here from env to file
    #dist.init_process_group(backend='gloo', init_method = 'file:///E:/WS2324/praktikum/SynDiff/SynDiff/some_file', rank=rank, world_size=size)
    dist.init_process_group(backend='nccl', rank=rank, world_size=size)
    print("after calling init_process_group")
    print("before calling train_syndiff")
    fn(rank, gpu, args)
    print("after calling train_syndiff")
    dist.barrier()
    cleanup()  
...
if __name__ == '__main__':
    ...
    with wandb.init(project="syndiff", config=args.__dict__):
        if size > 1:
            processes = []
...
def main(config=None):
    ...
    with wandb.init(config=config):
        args=wandb.config
        size = int(os.getenv('SLURM_NTASKS'))//int(os.getenv('SLURM_JOB_NUM_NODES'))
        if size > 1:#multi GPUs
            print("size>1")
            processes = []
            for local_rank in range(size):
                #args.local_rank = rank----------yichen modified
                print('Node rank %d, local proc %d, global proc %d' % (0, local_rank, int(os.getenv('SLURM_PROCID'))))
                p = Process(target=init_processes, args=(local_rank,int(os.getenv('SLURM_PROCID')), int(os.getenv('SLURM_NTASKS')), train_syndiff, args))
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
            ...
            print("size==1")
            init_processes(0,int(os.getenv('SLURM_PROCID')), int(os.getenv('SLURM_NTASKS')), train_syndiff, args)
